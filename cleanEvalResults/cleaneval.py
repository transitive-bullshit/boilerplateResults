...
def normalize(text, ascii=False, unlabelled=False):
    ...
    if unlabelled:
        text = re_TAG.sub(
            "\n<p> ", text
        )  # start each segment on new line, normalise tags
    else:
    ...
    if ascii:
        text = re_HI.sub(
            "", text
        )  # delete non-ASCII characters (to avoid charset problems)
...
def evaluate(diff):
    ...
    precision_plus_recall = (
        tag_precision + tag_recall if tag_precision + tag_recall > 0 else 1
    )
...
def write_alignment(diff, filename):
    with open(filename, "w") as file:
        for tag, text_seg, gold_seg in diff:
            if tag == "delete":
                print("<" * 40, "(false positive)", file=file)
                print(" ".join(text_seg), file=file)
            if tag == "insert":
                print(">" * 40, "(false negative)", file=file)
                print(" ".join(gold_seg), file=file)
            if tag == "equal":
                print("=" * 40, file=file)
                print(" ".join(gold_seg), file=file)
...
for filename in files:
    ...
    if not opt_total:
        print(
            filename
            + "\t"
            + ("%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%d\t%d\t%d\t%d\t%d\t%d" % eval_list)
        )
...
if n_processed < n_files:
    print(
        "Warning: %d files not found in gold standard (skipped)"
        % (n_files - n_processed),
        file=sys.stderr,
    )
...
if opt_summary:
    ...
    if n_processed < n_files:
        print(
            "Evaluation report for on %d out of %d files:" % (n_processed, n_files),
            file=sys.stderr,
        )
    else:
        print("Evaluation report for on %d files:" % (n_files), file=sys.stderr)
    print("Word-level accuracy (macro-averaged):", file=sys.stderr)
    print("  F-score:   %6.2f%% %s %.2f%%" % (mean[0], "+/-", sd[0]), file=sys.stderr)
    print("  Precision: %6.2f%% %s %.2f%%" % (mean[1], "+/-", sd[1]), file=sys.stderr)
    print("  Recall:    %6.2f%% %s %.2f%%" % (mean[2], "+/-", sd[2]), file=sys.stderr)
    print("Segmentation accuracy (%s):" % (is_labelled), file=sys.stderr)
    print("  F-score:   %6.2f%% %s %.2f%%" % (mean[3], "+/-", sd[3]), file=sys.stderr)
    print("  Precision: %6.2f%% %s %.2f%%" % (mean[4], "+/-", sd[4]), file=sys.stderr)
    print("  Recall:    %6.2f%% %s %.2f%%" % (mean[5], "+/-", sd[5]), file=sys.stderr)
    print("Word-level accuracy (micro-averaged):", file=sys.stderr)
    print("  F-score:   %6.2f%%" % (100 * f_score), file=sys.stderr)
    print("  Precision: %6.2f%%" % (100 * precision), file=sys.stderr)
    print("  Recall:    %6.2f%%" % (100 * recall), file=sys.stderr)
    print(
        "Segmentation accuracy (%s, micro-averaged):" % (is_labelled), file=sys.stderr
    )
    print("  F-score:   %6.2f%%" % (100 * tag_f_score), file=sys.stderr)
    print("  Precision: %6.2f%%" % (100 * tag_precision), file=sys.stderr)
    print("  Recall:    %6.2f%%" % (100 * tag_recall), file=sys.stderr)
